...
describe("allowance cleanups", () => {
  /**
   * `allowance-reset.ts` states the invariant its reset logic leans on: every
   * plan ends by returning the allowance to zero, so within this server's own
   * flows the next approval never collides. Enforcing it centrally is what
   * makes that true of the tools that never cleaned up after themselves.
   */
  it("returns every allowance it granted to zero", () => {
    const result = executionPlanFromSteps({
      chainId: "1",
      sender,
      steps: [
    ...
    expect(result.ordered_steps.map((step) => step.kind)).toEqual([
      "approval",
      "execution",
      "allowance_cleanup",
    ]);
    expect(result.ordered_steps[2]?.transaction.to).toBe(token);
    expect(result.ordered_steps[2]?.transaction.data).toBe(approvalCalldata(0n));
  });

  it("does not zero an allowance the tool already zeroed", () => {
    const result = executionPlanFromSteps({
      chainId: "1",
      sender,
      steps: [
    ...
    expect(result.ordered_steps).toHaveLength(3);
  });

  /**
   * The case this exists for. USDT refuses to replace a nonzero allowance, so
   * it needs the reset in front; leaving a remainder behind is what makes the
   * next plan's approval revert, so it needs the cleanup after. One plan
   * carries both, and neither the tool nor the reset logic has to know about
   * the other.
   */
  it("both resets and clears a token that rejects an overwrite", () => {
    const result = executionPlanFromSteps({
      chainId: "1",
      sender,
      steps: [
    ...
    expect(
      result.ordered_steps.map((step) => [step.kind, step.transaction.data]),
    ).toEqual([
      ["approval", approvalCalldata(0n)],
      ["approval", approvalCalldata(1_000n)],
      ["execution", "0xfeed"],
      ["allowance_cleanup", approvalCalldata(0n)],
    ]);
  });

  it("leaves a plan that grants nothing untouched", () => {
    const result = executionPlanFromSteps({
      chainId: "1",
      sender,
      steps: [
  ...
  it("clears each granted token once, whatever the grant order", () => {
    const otherToken = "0x4444444444444444444444444444444444444444" as const;
    const result = executionPlanFromSteps({
      chainId: "1",
      sender,
      steps: [
    ...
    expect(
      result.ordered_steps
        .filter((step) => step.kind === "allowance_cleanup")
        .map((step) => step.transaction.to),
