...
Aerodrome skill: ekubo://skills/use-aerodrome (${origin}/skills/use-aerodrome/SKILL.md)
...
- This MCP does not proxy, index, cache, authenticate to, or replay Aave, Morpho, Sky, Lido, or Aerodrome data sources. The agent reads them directly, uses them to choose an action, then supplies explicit identifiers and safety bounds to a local preparation tool.
...
- Aerodrome: read ekubo://skills/use-aerodrome. Aerodrome is Base-only and has no data API; its Sugar lens contracts are the data pipeline and answer eth_call. Call prepare_aerodrome_sugar_reads for the dataset you need and run the returned bundle through the user's wallet/RPC, then feed the pool, gauge, fee, and bribe addresses it returns into the prepare_aerodrome_* tools rather than deriving them. Velodrome's published SDKs carry Optimism addresses and a drifted position struct, so cross-check contracts against get_aerodrome_deployment. A veNFT votes once per weekly epoch and a second attempt reverts; staking an LP token trades its trading fees for AERO emissions rather than adding to them.
...
ve(3,3): call get_ve33_allocations before reorganizing votes, show the complete allocation and state_id, validate its read-only multicall, then pass that state_id and at most 25 target weight_bps values to prepare_ve33_reallocation. Preserve the exact returned atomic order. Use the other dedicated tools for explicit extension, fee claims, vote clearing, and phased reinvestment. Read ekubo://docs/ve33-workflow before constructing a plan.
Removing vote weight: use prepare_ve33_clear_vote with each stake's current pool key. It claims that pool immediately before clearing the vote, because Ve33 discards pending voter fees when a stake's weight goes to zero, and a pool key that is not the stake's active one reverts the batch before anything clears. The stake amount, lock end, and NFT ownership are unchanged and the vote can be re-applied later; the pool loses that weight, and a pool with no remaining vote weight charges a zero extension fee. Report both before authorizing.
