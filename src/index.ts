        ...
        return json(
          {
            ...
            operational_semantics: {
              mcp_tool_result_storage: `wallet_payload_bodies_only: prepared execution plan bodies, read-call bundles (exact wallet_batch_eth_call argument objects), and token lists requested with as_reference are retained at ${url.origin}/artifact/<id> for ${ARTIFACT_TTL_SECONDS} seconds so wallets fetch them by reference instead of receiving them through the agent; no other tool results are stored or replayed`,
              artifact_delivery: {
                mode: "reference",
...
- Execution plan bodies, read-call bundles (exact wallet_batch_eth_call argument objects), and token lists requested with as_reference are stored at ${origin}/artifact/<id> and returned as artifact_reference envelopes under execution_plan_reference, read_calls_reference, and token_list_reference. Pass the whole envelope unchanged as the wallet tool's reference argument; the wallet fetches the body itself and verifies integrity. A 404 means the reference expired: re-run the tool that produced it. No other tool results are stored or replayed.
