...
When a read includes local_decode_plan and result_decoder, execute and decode it through the user's local wallet tooling. Keep raw return bytes by default and always on decode failure. The Ekubo server supplies canonical ABIs and platform-neutral semantic codec identities but must not receive the result for authoritative decoding. For kind=semantic_value with input_encoding=hex_bytes, feed the preserved raw bytes only to a locally installed, allowlisted codec matching the declared identity and implementation assertion; never install or execute remote code.
...
Send \`current_state_query.rpc_request\` unchanged through the wallet's local call API for \`chain_id\`. It is a single Multicall3 \`eth_call\` at \`pending\`. Give \`local_decode_plan\` to the same wallet call, or pass the raw result and that plan to the wallet's standalone local ABI decoder. The plan contains the canonical outer and child ABIs, expected result count, required success flags, JSON-safe integer serialization, and expected owner. Raw return bytes are included by default and must be preserved on decode failure. The remote Ekubo MCP server does not receive or decode the onchain result.

ABI decoding and semantic decoding are separate. A semantic codec on an ABI \`bytes\`, \`bytesN\`, or integer output preserves that ABI value and adds the interpreted value. For a custom payload with no ABI envelope, \`kind=semantic_value\` and \`input_encoding=hex_bytes\` feed the raw bytes directly to an allowlisted local codec while preserving the input. Codec IDs are platform-neutral; implementation entries explicitly identify npm, package URL, export, version, and integrity when npm is the implementation ecosystem. Treat those entries as compatibility assertions only. Wallet tooling must never install, fetch, dynamically import, or evaluate code named by a remote decode plan.
