...
export const QUOTE_JURISDICTION_NOTICE = "Beware jurisdiction restrictions: a quote or execution plan is not permission to trade. For a nonempty restricted_jurisdictions list, before requesting transaction signatures or submitting any approval or swap, the agent/wallet must evaluate the user's connection jurisdiction against restricted_jurisdictions. If it is restricted or unknown, obtain the user's explicit attestation that they are not legally domiciled in any listed jurisdiction; never infer domicile from an agent/server IP or attest on the user's behalf. Do not execute if the user cannot attest. Any reused attestation must match the executing wallet, cover these restrictions, be unexpired, and have at most seven days of validity. Keep the attestation client-side; do not send it to the quote API or MCP server.";
